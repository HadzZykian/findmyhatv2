...
function startGame() {
    ...
    while (isPlaying) {     //(isPlaying == true)
        ...
        if  (currCharRow < 0 || currCharCol < 0 || currCharRow > height-1 || currCharCol > width-1) {
            console.log('Out of bounds! Game Over! Dont fall off the field!');
            isPlaying = false;
        }
        else if (field[currCharRow][currCharCol] == hat) {
            console.log('Congrats, you found your hat! You win the game! Now you can graduate!');
            isPlaying = false;
        }
        else if (field[currCharRow][currCharCol] == hole) {
            console.log('Sorry, you fell down a hole! Please try again till you win!');
            isPlaying = false;
        }
        field[currCharRow][currCharCol] = pathCharacter;
